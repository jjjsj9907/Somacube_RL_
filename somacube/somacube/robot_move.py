...
package_path = get_package_share_directory("somacube")
...
try:
    from DSR_ROBOT2 import movej, movel, get_current_posx, mwait, trans, DR_BASE
...
class RobotController(Node):
    ...
    def robot_control(self):
        ...
        if user_input:
            ...
            if depth_future.result():
                ...
                self.somacube_target(target_pos)
                self.init_robot()
        self.init_robot()
    ...
    def somacube_target(self, target_pos):
        # delete
        ...
        movel(up_pos(target_pos_up, 3, -10), vel=VELOCITY, acc=ACC)
        # movel(BUCKET_POS, vel=VELOCITY, acc=ACC)
        mwait()
        gripper.close_gripper()
        movej(HOME, vel=VELOCITY, acc=ACC)
        
        movel(up_pos(re_grap_pos, 3, 300), vel=VELOCITY, acc=ACC)
        movel(re_grap_pos, vel=VELOCITY, acc=ACC)
