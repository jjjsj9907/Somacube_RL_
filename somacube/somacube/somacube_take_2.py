...
def get_true_visual_origin(piece_coords, position):    
    ...
    return [abs_x_list[0], abs_y_list[0], abs_z_list[0], average_x]
...
def get_zyz_angles(piece_id, orientation_index):
    ...
    src = BASE_PIECES[piece_id]
    dst = ALL_PIECE_ORIENTATIONS[piece_id][orientation_index]
        # 중심 정렬 (translation 무시)
...
class RobotTester:
    ...
    def _select_action(self, state, piece_id, env, epsilon = 0.0):
    ...
    def test_robot_control(self, num_tests=1):
        ...
        for test in range(num_tests):
            ...
            while not env.done and steps < 20:
                ...
                action = self._select_action(state, piece_id, env,epsilon = 0.0)
